# This is the main python file to perform data processing with PySpark (LOCAL)
...
def main():
    ...
    """
    df_tempview = spark.sql(query)
    print(df_tempview.count())

    # Check size of output dataframe, comment if not needed
    # print("After: ", df_tempview.count())

    # Export file to 1 csv file
    df_tempview.coalesce(1).write.option("header", "true").csv("results/Jan2022_cleaned.csv")

    # This writes to mulitple csv file (based on # of partitions)
    # df_tempview.write.csv("results/sep2022_sample_partition_optimized.csv", header=True, sep=',')
