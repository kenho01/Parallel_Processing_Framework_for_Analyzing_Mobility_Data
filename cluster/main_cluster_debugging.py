# This is the main python file to perform data processing with PySpark (Debugging)
...
def main():
    ...
    conf = SparkConf().setAppName("Mobility Data Process Optimisation Dataset Partitioned (June2022)")
    spark = SparkSession.builder.config(conf=conf).getOrCreate()
    conf = spark.sparkContext.getConf()

    file_path = "s3a://fyp-ken/raw_data/June2022.csv"
    raw_rdd = spark.sparkContext.textFile(file_path)

    # Initial count
    # initial_count = raw_rdd.count()
    ...
    # Remove trips with invalid DT_START and DT_END format
    ...
    # Cast datetime from string to timestamp, latitude & longitude to float, call UDF to check for off-boundary points, removing NULL latitudes & longitudes
    query = """
    SELECT 
    ...
    # Calculate distance travelled and time taken for each trip 
    ...
    # Remove anomalies: Distance traveled < 1km, create new column for partition
    ...
    final_count = df_tempview.count()

    # Check size of output dataframe, comment if not needed
    # print("After: ", df_tempview.count())

    # Export file to 1 csv file
    # df_tempview.coalesce(1).write.option("header", "true").csv("s3a://fyp-ken/results_combined/Jan2022_test")

    # This writes to mulitple csv file (based on # of partitions)
    df_tempview.write.csv("s3a://fyp-ken/results_partitioned/June2022", header=True, sep=',')

    # This writes to year-month partition
    # df_tempview.write.option("header", "true").partitionBy("year_month").csv("s3a://fyp-ken/results/Jan_to_Feb2022_partitioned")
    ...
    # print("Initial dataframe size: ", initial_count)
    print("Final dataframe size: ", final_count)
